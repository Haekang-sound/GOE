...
void Application::Initialize()
{
	...
	UIInitInfo uiInfo;
	uiInfo.commandQueue = dynamic_cast<GOERenderer*>(m_renderer.get())->GetCommandQueue();
	uiInfo.device = dynamic_cast<GOERenderer*>(m_renderer.get())->GetDevice();
	uiInfo.frameBufferCount = dynamic_cast<GOERenderer*>(m_renderer.get())->GetFrameCount();
	uiInfo.imguiDescriptorHeap = dynamic_cast<GOERenderer*>(m_renderer.get())->GetDescriptorHeap();
	m_editor = std::make_unique<EditorCore>(m_hWnd); // EditorCore 인스턴스 생성
	m_editor->Initialize(&uiInfo);
}
...
int Application::Run()
{
	while (true)
	{
		...
		uiInfo.imguiDescriptorHeap = dynamic_cast<GOERenderer*>(m_renderer.get())->GetDescriptorHeap();
		uiInfo.rendertarget = dynamic_cast<GOERenderer*>(m_renderer.get())->GetCurrentRendertarget();
		m_renderer->OnRender(); // 렌더링 호출
		m_editor->OnRender(&uiInfo); //반드시 Renderer다음에 호출 할것
